...
<div class="post-code">
    > import time <br>
    > <br>
    > <span># 시작시간 체크</span><br>
    > code_start = time.time() <br>
    > <br>
    > <span># 동작할 내용 여기에 추가</span><br>
    > <br>
    > <span># 완료시간 체크</span><br>
    > code_end = time.time()<br>
    > <br>
    > <span># 총 실행시간 표시 (소수점 5자리까지)</span><br>
...
<div class="post-code">
    ...
    > <span># (1) input()으로 입력받은 한 줄을 split()으로 나눈다.</span><br>
    > <span># (2) map을 이용해서 (1)에서 입력받은 내용을 int로 캐스팅한다.</span><br>
    > &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span>map(함수명, 반복가능 자료형)</span><br>
    > <span># (3) (2)에서 작업한 내용을 list로 저장한다.</span>
</div>
<br>
...
<div class="post-code">
    ...
    > <span># 입력 개수에 알맞게 변수 설정</span><br>
</div>
<br>
🔸input()보다 입력이 빠른 sys.stdin.readline()<br>
<br>
<div class="post-code">
    > <span># sys 라이브러리 추가</span><br>
    > import sys<br>
    > <br>
    > a = sys.stdin.readline().rstrip()<br>
    > <span># readline 사용시 개행문자 제거를 위해 rstrip 필수</span><br>
    > <span># readline에 정수를 인자로 주면 해당 글자수 만큼 입력</span><br>
</div>
<br>
...
<br>
<div class="post-code">
    > <span># list = [[0] * x] * y 로 선언시</span><br>
    > <span># [0] * x 부분이 다 같은 오브젝트로 처리됨 (얇은 복사)</span><br>
    <br>
    > <span># 다음과 같이 처리 필요</span><br>
